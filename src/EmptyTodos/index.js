...
function EmptyTodos(props){
  // Declara la variable imgDataURI antes de usarla
  ...
  // Usa useEffect para manipular el DOM después de que el componente se monte
  React.useEffect(() => {
    // Selecciona el elemento con la clase "bounce" y cambia su background-image
    const bounceElement = document.querySelector(".bounce");
    if (bounceElement) {
      bounceElement.style.backgroundImage = `url(${imgDataURI})`;
    }
    ...
    <div className='container-empty-todos'>
      <div className="empty-icon-container">
        ...
        <div>
          ...
          {(!!props.totalTodos) && (
            <p>Sorry! We couldn't find any results for {props.searchText}</p>
          )}
          {(!props.totalTodos) && (
